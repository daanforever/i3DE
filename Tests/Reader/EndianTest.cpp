...
namespace lorem::reader::EndianTests {
  ...
  void ConstructorWithIntance(Endian::Endianness endian) {
  ...
  TEST(EndianTest, ConstructorEndian) {
    {
      ...
      ConstructorWithIntance(Endian::Endianness::Little);
    }
    {
      SCOPED_TRACE("Big endian");
      ConstructorWithIntance(Endian::Endianness::Big);
    }
  }
  ...
  //TEST(EndianTest, get0) {
  //  ASSERT_NO_THROW(Endian().open(simple_file_ptr).get(0));
  //  ASSERT_EQ(0, Endian().open(simple_file_ptr).get(0).size());
  //}

  TEST(EndianTest, getOne) {
    //Endian().open(simple_file_ptr).get(1);

    auto& result = Endian().open(simple_file_ptr);
    ASSERT_NO_THROW(result.get(1));
  ...
  void getNum1Testing(T value) {
    ASSERT_NO_THROW(Endian(Endian::Endianness::Big).open(simple_file_ptr).get<T>(1));

    auto endianStream = Endian(Endian::Endianness::Big).open(simple_file_ptr);
  ...
  //  ASSERT_NO_THROW(Endian(Endian::Endianness::Big).open(simple_file_ptr).get<std::uint64_t>(1));

  //  auto endianStream = Endian(Endian::Endianness::Big).open(simple_file_ptr);

  //  ASSERT_EQ((std::uint64_t)0x3132333400000000, endianStream.get<std::uint64_t>(1).front());
  //}
  
  //TEST(EndianTest, getBigEndianFloat) {
  //  ASSERT_NO_THROW(Endian(Endian::Endianness::Big).open(simple_file_ptr).get<float>(1));

  //  auto endianStream = Endian(Endian::Endianness::Big).open(simple_file_ptr);

  //  ASSERT_EQ(1, endianStream.get<float>(1).front());
  //}
